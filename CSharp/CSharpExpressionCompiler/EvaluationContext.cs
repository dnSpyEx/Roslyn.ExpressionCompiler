...
namespace Microsoft.CodeAnalysis.CSharp.ExpressionEvaluator
{
    internal sealed class EvaluationContext : EvaluationContextBase
    {
        ...
        internal static EvaluationContext CreateMethodContext(
            ...
            => CreateMethodContext(metadataBlocks, getMethodDebugInfo, new ModuleId(moduleId, "<unknown>"), methodToken, methodVersion, ilOffset, localSignatureToken);
            ...
            int localSignatureToken)
        {
            ...
            var currentSourceMethod = compilation.GetSourceMethod(debugInfo.Compiler, moduleId, methodHandle);
            ...
            out ImmutableArray<string> errorMessages)
        {
            ...
            bool isInFieldKeywordContext = context.IsInFieldKeywordContext(_methodDebugInfo.Compiler);
            ...
            CompilationTestData? testData)
        {
            ...
            var syntax = Parse(expr, context.IsInFieldKeywordContext(_methodDebugInfo.Compiler), (compilationFlags & DkmEvaluationFlags.TreatAsExpression) != 0, diagnostics, out var formatSpecifiers);
            if (syntax == null)
            {
        ...
        {
            var context = CreateCompilationContext();
            var assignment = target.ParseAssignment(expr, context.IsInFieldKeywordContext(_methodDebugInfo.Compiler), diagnostics);
            if (assignment == null)
            {
                resultProperties = default;
                return null;
